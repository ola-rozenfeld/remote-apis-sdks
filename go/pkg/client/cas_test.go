...
const (
	...
	reqMaxSleepDuration   = 10 * time.Millisecond
)
...
func TestUploadConcurrentBatch(t *testing.T) {
	...
	for i, blob := range blobs {
		...
		if fake.BatchReqs() != 2 {
			t.Errorf("%d requests were made to BatchUpdateBlobs, wanted 2", fake.BatchReqs())
		}
	}
}
		...
		t.Run(fmt.Sprintf("%sUsingBatch:%t", t.Name(), ub), func(t *testing.T) {
			...
			if bool(ub) {
				...
				if fake.BatchReqs() > 50 {
					t.Errorf("%d requests were made to BatchReadBlobs, wanted <= 6", fake.BatchReqs())
				}
			}
		})
	}
